MovieWatchlist
Selamat datang di repositori resmi MovieWatchlist!MovieWatchlist adalah aplikasi mobile berbasis Android yang memungkinkan pengguna untuk melacak daftar film yang ingin ditonton atau telah ditonton. Dibangun sebagai proyek UAS Pemrograman Berbasis Mobile, aplikasi ini menggunakan Java dan Supabase untuk menyediakan pengalaman pengelolaan watchlist yang efisien dengan antarmuka modern bertema hitam dan merah.
...
Arahkan ke folder MovieWatchlist-UAS.

3. Konfigurasi Supabase

Buat proyek di Supabase.

Dapatkan URL API (e.g., https://<your-project-id>.supabase.co/rest/v1/) dan Anon Key dari Settings > API di dashboard Supabase.

Buat file Constants.java di app/src/main/java/com/moviewatchlist/utils/:
...
Buat tabel movies di Supabase dengan skema berikut:
CREATE TABLE movies (
...
Aktifkan Row-Level Security (RLS) dan tambahkan kebijakan:
...
Untuk mengimpor ke Supabase:psql -U postgres -d your_database -f database/movies_dump.sql
...
Buka app/build.gradle dan pastikan dependensi berikut ada:implementation 'androidx.appcompat:appcompat:1.6.1'
...
Hubungkan perangkat Android atau jalankan emulator (e.g., Pixel 6 API 33).
Klik tombol Run (ikon segitiga hijau) atau jalankan perintah:./gradlew installDebug


Aplikasi akan memulai di MainActivity dengan HomeFragment sebagai layar utama.


▶️ Cara Penggunaan

Login: Masuk dengan kredensial Supabase melalui LoginActivity.
Lihat Daftar Film: Buka HomeFragment untuk melihat watchlist dalam RecyclerView.
Tambah Film: Klik tombol "Add" untuk membuka AddMovieFragment, lalu masukkan detail film (judul, genre, deskripsi, status, rating, poster URL opsional).
Lihat Detail Film: Klik kartu film untuk membuka MovieDetailActivity dengan detail lengkap.
Edit Film: Klik ikon edit pada kartu film untuk membuka EditMovieFragment dan perbarui detail.
Hapus Film: Klik ikon hapus pada kartu film untuk menghapus dari watchlist.
Profile: Buka ProfileFragment untuk melihat email pengguna atau logout.
Navigasi: Gunakan tombol kembali atau tutup untuk berpindah antar layar.
...
Cek Glide: Verifikasi konfigurasi Glide di MovieAdapter. Tambahkan logging untuk debugging:Glide.with(context)
...
Sofi (sofi@email.com)
[Nama Anggota 2] (email2@email.com)
...
Terima kasih telah menggunakan MovieWatchlist! 🎬Untuk pertanyaan atau dukungan, hubungi sofi@email.com.
